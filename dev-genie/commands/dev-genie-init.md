...
3. **Walk the registry in order.** For each marketplace plugin entry — `guardrails`, `audit`, then optional `katana` and `daimyo`:
   a. Run the entry's **install check**. If the plugin is not reachable, instruct the user to install it from the dev-genie marketplace and pause until they confirm. Marketplace plugins are pulled from `main` and launched from their own plugin folder; dev-genie must not run package-manager installs for sibling plugins or internal libraries.
   ...
   d. **Invoke the sub-plugin's own setup command**, do not re-implement its logic:
      ...
      - For `katana`: offer `katana install <platform>` or the platform's registered Katana plugin flow only if the project wants Katana-managed work documents.
      - For `daimyo`: verify the Daimyo plugin/CLI/MCP server is reachable. No project mutation is required unless the user opts into a specific long-running Loop workflow.
