...
describe("ArchitectRoleRunner", () => {
  ...
  it("returns skipped for unsupported Architect version, operation, or missing required output", async () => {
    const client = new StubStructuredModelClient(validArchitectureImpact());
    const runner = new ArchitectRoleRunner({
      modelClient: client,
      now: fixedNow,
    ...
    const unsupportedOperation = await runner.run(withPayload({ operation: "review_architecture" }));
    ...
    expect(missingOutput.payload.skip_reason?.code).toBe("role:no_required_architecture_impact");
    expect(client.calls).toHaveLength(0);
  });
  ...
  it("resolves a generic protocol validator by artifact type", () => {
    const validator = validatorFor("ArchitectureImpact");

    expect(validator(validArchitectureImpact())).toBe(true);
  });
});
...
function validArchitectureImpact(): JsonObject {
  return {
    ...
    output_refs: [],
    ownership: {
      owns_files: ["roles/src/runner/role-runner.ts"],
      owns_interfaces: ["interface:role-runner"],
      owns_data: [],
      owns_workflow_steps: ["workflow:roles-architect-role"],
    },
    confidence: {
      score: 0.88,
      level: "high",
    ...
    payload: {
      ...
      proposed_changes: [
        {
          change_id: "add_shared_role_runner",
          change_type: "add",
          component: {
            name: "RoleRunner",
            kind: "role",
