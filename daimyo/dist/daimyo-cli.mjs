...
import { access, mkdir as mkdir3, open, readFile as readFile3, readdir, truncate } from "node:fs/promises";
...
var JsonlExecutionStore = class {
  ...
  async appendEvent(taskId, event) {
    await mkdir3(this.executionDir, { recursive: true });
    const filePath = this.taskLogPath(taskId);
...
async function main(argv = process.argv.slice(2)) {
  ...
  const daimyo = createStandaloneDaimyo({
    ...
    model: {
      ...
      ...options.apiKeyEnv === void 0 ? {} : { apiKeyEnv: options.apiKeyEnv }
    },
    ...options.cwd === void 0 ? {} : { cwd: options.cwd }
  });
  const taskId = options.task === void 0 ? await firstRunnableTaskId(daimyo.workSource) : options.task;
