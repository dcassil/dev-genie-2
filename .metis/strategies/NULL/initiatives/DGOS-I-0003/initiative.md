...
The existing Planner spec describes the right role: classify work, select strategy recipes, identify dependencies and parallel work, and emit structured planning artifacts. This must become a deterministic engine plus bounded reasoning passes, not another pile of prompt-only workflows.

## Goals & Non-Goals

**Goals:**
- Define work patterns for greenfield, feature, bug, refactor, migration, port, and mixed work.
- Make strategy recipes declarative: required inputs, produced artifacts, plugin routes, validation gates, and skip conditions.
...
Planner should output a PlanningPass artifact with work altitude, pattern, mode, required artifacts, missing artifacts, dependencies, parallel groups, risks, human decisions, and next plugin routes.
